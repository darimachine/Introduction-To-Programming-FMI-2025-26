...
- if, || и && работят само с булеви стойности, като небулевите стойности(като например числа, ако направим if(5)) имплицитно, или скрито, се превръщат в булеви стойности преди да бъдат оценени
- switch оценява подадената стойност, без значение дали е булева, число или символ, без да я конвертира, сравнявайки със стойността, подадена във всеки case
</details>
...
Имаме глобална променлива value и локална променлива value вътре в main(), като тук main() е по-вътрешен scope в сравнение с глобалния -> няма проблем да си създаде друго value, което shadow-ва глобалното.<br><br>

Първите два cout-a изписват "3\n42\n", взимайки съответно стойностите на локалната и глобалната променлива.<br><br>

Влизаме в f(), където подаваме value по стойност, т.е. вътре в f() се прави негово копие, а оригиналното value, което е локално за main(), няма как да се промени в f(). Вътре в f() задаваме стойност 9 на подаденото копие, която присвояваме на глобалната променлива, която извикваме чрез оператора за резолюция(::) -> след f() не сме променили локалното value в main(), но на глобалното сме задали стойност 9.<br><br>

Вторите два cout-a изписват "9\n3\n".
...
<details>
    <summary>Обяснение</summary>
Не се компилира. main() не знае, че f() съществува, понеже f() е дефинирана след тялото на main(). Трябва или да изнесем дефиницията на f() над main(), или просто да я декларираме над main().
</details>
...
<details>
    <summary>Обяснение</summary>
Не се компилира. Постфиксен оператор връща rvalue, което е стойност без адрес, на която няма как да се присвои друга стойност.
</details>
...
<details>
    <summary>Обяснение</summary>
В нито един случай няма break -> в който и да влезем, ще пропаднем до последния.<br><br>

Няма ограничение за реда, в който изреждаме случайте - default може да е както първи, така и последен, така и на всяка друга позиция. Също така не сме задължени да слагаме break в нито един случай, за разлика от други езици.<br><br>

Естествено, понеже пропадането по случаите винаги е надолу, това как ще ги наредим може да се отрази на резултата от извикването на даден switch, ако имаме пропадания.
</details>
...
<details>
    <summary>Обяснение</summary>
i = 5 присвоява стойността 5 на i, след което оценява i-то, което вече има стойност 5, т.е. се оценява с true. Така проверката винаги ще връща истина и цикълът ще е безкраен, като във всяка негова итерация ще се принтира "5 ".
</details>
...
Тук ключовите стойности са 1 и 3.<br>

- a == 1 -> изписва се "one13", понеже влизаме в двата if-a.
- a == 3 -> изписва се само "error", понеже влизаме само в else-а.
- a != 1 && a != 3 -> изписва се само "13", понеже влизаме само във втория if. Тъй като това множество от стойности за 'а' е безкрайно, това ще е нашият default случай. Можем да забележим също, че това "13", което се изписва в този случай, се изписва и в случая 'а == 1', т.е. е възможно пропадане.<br>

Тук е важно да се забележи, че в else if-а няма как да влезем, понеже дотам стигаме само ако 'а == 3'(т.е. когато не сме минали проверката 'а != 3' в горния if). Но ако 'а == 3', няма как 'а > 3' -> от else if-а няма смисъл в този код и можем да го игнорираме.<br><br>
...
switch(a) {
    case 1: cout << "one";
    default: cout << "13"; break;
    case 3: cout << "error";
}

switch(a) {
    case 1: cout << "one13"; break;
    default: cout << "13"; break;
    case 3: cout << "error"; break;
}
```
...
Ето три валидни варианта - първите два са с пропадане, а третият - без, като в неговия случай за 'а == 1' се принтира цялото "оne13".
...
В един for цикъл, инкрементната стъпка се изпълнява след всяка итерация на цикъла и преди проверката на следващата итерация, 
значи тук ще се изпълнява след всяка инкрементация на i с i++ в тялото на цикъла и преди проверката i <= 16.

- първа итерация - i става 10, прави се инкрементната стъпка(изписва се 10 и нов ред), прави се проверката 10 <= 16
- втора итерация - i става 11, прави се инкрементната стъпка(изписва се 11 и нов ред), прави се проверката 11 <= 16
- ...
- седма итерация - i става 16, прави се инкрементната стъпка(изписва се 16 и нов ред), прави се проверката 16 <= 16 (за последен път е true, значи ни остава една итерация и инкрементата стъпка, която идва след нея)
- последна итерация - i става 17, изписва се в инкрементната стъпка, прави се проверката 17 <= 16, която завършва цикъла
...
<details>
    <summary>Обяснение</summary>
Отново имаме опит за присвояване на стойност на rvalue, което не се компилира.
</details>
...
- първа итерация - прави се проверката i++, която връща -10, но прави i=-9, изписва -9, инкрементната стъпка прави i=-8

Тук можем да забележим, че ако числото, с което сме влезли в текущата итерация на цикъла, е a0, проверката се извършва, оценявайки това а0, после се изписва 'a1 = a0 + 1'(първото по-голямо нечетно), и в следващата итерация на цикъла влизаме с 'a2 = a1 + 1 = a0 + 2'(първото по-голямо четно).<br>

- оценяваме -10 и изписваме -9
- оценяваме -8 и изписваме -7
- оценяваме -6 и изписваме -3
- оценяваме -4 и изписваме -5
- оценяваме -2 и изписваме -1
- оценяваме 0 и излизаме от цикъла
